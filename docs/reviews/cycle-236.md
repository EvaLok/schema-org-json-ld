...
**Evidence**: The entry title is “Cycle 236 reflections,” the first context sentence is “Cycle 236 focused on Cycle 236 reflections” (`:160-177`), and it contains two separate `### Context` sections. Its follow-through section says commitment 2 was followed because cycle 236 “verified 4 receipts for cycle 236” (`:168-173`), but canonical `cycle-receipts` output returns 6 receipts. The closing commitments are also not objectively checkable: “Close audit-inbound #1115 after audit #214 is processed” and “Monitor for new audit/QC communications” (`:191-194`).
...
**Evidence**: The summary table counts `29` Rust tool crates, `1` shell-only tool, `1` TypeScript-only tool, and `1` Rust library crate, then claims `**Total tools** | **31**` (`:10-17`). Those category counts add up to 32, not 31. The artifact’s own methodology later says `state-schema` is excluded because it is a library, not a tool (`:79-83`), so including it in the summary while also claiming a 31-tool total is internally inconsistent. The discrepancy section nonetheless says “None blocking” (`:66-69`).
...
**Evidence**: The cycle 235 `review_agent.history` entry marks `deferred-remediation` with `"disposition": "deferred"` (`:4930-4932`), but the same evidence string says `#1110 PR #1111 merged via b05b8d9; receipt-tooling finding resolved` (`:4932`) and the note repeats that it is a “deferred classification but underlying issue resolved” (`:4936`). That is internally inconsistent state: a finding with concrete merge evidence and a declared resolution is being recorded as deferred. At the same time, the same history entry credits `audit-evidence` as actioned via `1235a9d` (`:4925-4927`), but that artifact itself contains the count contradiction above.
