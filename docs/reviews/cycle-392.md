...
**Evidence**: The code still codifies the previous-cycle step-comment backstop as a `Warn` that can leave the overall pipeline result as `Pass`. Cycle 392 hit that exact behavior in production: the final C5.5 gate comment on issue `#1903` reported `step-comments` against `issue #1896` with `25` unique step comments, while `current-cycle-steps` in the same raw JSON correctly checked `issue #1903`. A direct count on the cycle 392 issue comments shows `26` unique step comments on `#1903` (`0` through `C8`, including `C6.5`). The process gate therefore validated the wrong issue thread and understated the cycle's actual step-comment coverage.
