...
**Evidence**: The cycle 463 carry-forward commitment explicitly required that PR `#2322`/PR `#2323` be "merged via merge-pr". But the cycle 464 worklog says PR `#2323` was "admin-merged via gh pr merge --squash --admin". Despite that miss on sub-criterion (a), the cycle 464 journal still records "DONE, all 7 sub-criteria pass post-merge". The follow-through grading therefore rewrote a binary observable after the fact instead of grading what actually happened.
**Recommendation**: Grade commitments against their literal observables. If a cycle had to use `gh pr merge --admin` instead of `merge-pr`, record that sub-criterion as not met and explain why, rather than upgrading it to a full pass.
...
**Evidence**: `docs/state.json` currently says cycle 464 `last_cycle.summary` is `"3 dispatches, 2 merges (PR #2323, PR #2325)"` with timestamp `2026-04-09T05:23:20Z`. But git shows `8b66a6e` (`state(cycle-complete): 2 dispatches, 2 merges`) was committed at `2026-04-09T05:23:20Z`, and the later review-dispatch commit `9915d5e` (`state(record-dispatch): #2331 dispatched [cycle 464]`) landed at `2026-04-09T05:27:13Z`. So the cycle mutated `last_cycle.summary` after close-out without advancing `last_cycle.timestamp`, recreating the exact backdating defect that cycle 463 review F4 had just identified and that cycle 464 had already dispatched in `#2327`.
...
**Evidence**: The cycle 464 worklog says worklog-accuracy and journal-quality were refreshed and moved to `in_progress`, and the journal says this refresh was needed to clear a blocking `chronic-category-currency` failure. But the state entries themselves mark `updated_cycle` and `verification_cycle` as `464` while their rationales admit the real structural response is only "planned as docs-lint crate dispatch in cycle 465". The same rationale also cites cycle 464 dispatch `#2327`, yet `#2327` is scoped only to the step-comments bug, timestamp coherence, and deferred-resolution merge gate — not to worklog-accuracy or journal-quality enforcement. The refresh therefore advanced verification markers without a merged or even dispatched fix for the chronic categories being marked fresh.
