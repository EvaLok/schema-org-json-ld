...
**Evidence**: `docs/state.json` shows that review issue `#1776` was dispatched during cycle 363, so close-out continued past the final gate. Issue `#1772`'s C5.5 step comment recorded a blocking failure: `overall: fail`, `has_blocking_findings: true`, and `doc-validation` failed. Even so, C6 dispatched the review and C8 posted `Pipeline: PASS`. Re-running `bash tools/pipeline-check --json --repo-root .` after the cycle still returns `overall: "fail"` with a blocking `doc-validation` failure. This was not a harmless warning; the cycle claimed success after a recorded blocking gate failure.
