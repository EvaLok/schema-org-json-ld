...
catalogue and extended it with cross-cluster intersections;
cycles 100-102 absorbed PAI + omx deeper reads adding 4 new
sub-shapes (A.10, A.11, H.5, H.6); cycle 104 integrated those
sub-shapes into the cross-cluster intersection layer (4 new
sub-patterns: A↔B 6, F↔H 6 + 7, A↔C 6 — total cross-cluster
sub-patterns 34 → 38). The mining + synthesis arc produced
**45 implications across 9 architectural clusters across 6 systems
(extended to 10 deep-dive systems by cycle 101)**, with seven
elevated cross-cluster intersection patterns (A↔B, F↔H, D↔I from
cycle 72; A↔C, B↔C, F↔I, E↔I from cycle 74; cycle 104 added 4
sub-patterns within those 7 intersections) showing how mechanisms
...
each containing multiple sub-shapes. The relationship between
clusters and Families is not 1:1 — many clusters intersect Family
patterns; some clusters (notably H, I) surface concerns the Family
layer didn't foreground. Substrate diversity at maximum across five
orthogonal axes (library/product/research-artifact, cloud/local,
enterprise/personal/research, software-development/game-environment,
...
**Cycle 73 mechanism evolution: cluster catalogue extracted to
`1-research/clusters.md`.** Cycles 62-72 added a deeper-pass
implications-mining catalogue (cluster A-I sub-shape catalogues, Phase 2
design-input mapping clusters to v1 failure modes, cross-cluster
intersection patterns from cycle 72 synthesis, open structural
questions, cycles 62-72 cadence summary) that grew the index file
from 736 lines (post cycle-33) to 1632 lines (post cycle-72) — past
the cycle-33 1422-line restructure trigger. Cycle 70 surfaced the
file-size pressure; cycle 72 deferred the structural restructure
(combining content addition with structural restructure in the same
cycle is risky for link breakage and editorial-decision compounding).
Cycle 73 (2026-05-05) executed the deferred restructure: the cluster
catalogue moved to [`1-research/clusters.md`](1-research/clusters.md),
mirroring the cycle-33 per-system-files split (independent
per-concern files; index keeps the load-bearing summary +
quick-reference cluster table + link). Index file dropped from 1632
lines back to a manageable size; cluster catalogue continues to grow
in its own file without re-triggering the index file-size trigger.
