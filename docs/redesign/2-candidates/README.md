...
| **B** | Per-role decision count is mitigated below v1 baseline despite ~3-4× sub-shape adoption | Cycle 92 (sharpened) → Cycle 96 (re-grounded post-PR#2878) → Cycle 103 (counting protocol specified, closing Risk 8 at specification level) | **enumeration-rubric-now-specified** (cycle 103 protocol uses structural-decision-points in artifacts as the primary unit, external observer enumeration, ratio-based threshold against v1 baseline measured under same rubric — replacing cycle 92's circular self-measurement) | **ambiguous-without-prototype-but-falsifiable (hard thresholds pre-agreed: PASS ≤ 1.0× v1; PASS-WITH-NOTE (1.0×, 1.5×]; PARTIAL-FLAG (1.5×, 2×]; REFUTED > 2×)** — cycle 92's "~80-110% of v1's" claim survives at PASS or PASS-WITH-NOTE under measurement; refuted at REFUTED. Phase 3 prototype measurement is the empirical step. |
