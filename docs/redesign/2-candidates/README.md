...
| **B** | Per-role decision count is mitigated below v1 baseline despite ~3-4× sub-shape adoption | Cycle 92 (sharpened) → Cycle 96 (re-grounded post-PR#2878) | **decision-class naming, not enumeration** (per-role counts are multiplier estimates dressed as bottom-up counts; v1 baseline is self-measured by the orchestrator that benefits from the comparison — circular per PR #2878 Finding 5/6) | **ambiguous-without-prototype (range 85-168% of v1 depending on cycle character)** — the cycle 92 "~80-110% of v1's" claim is the lower-bound case only; aggregate including coordination overhead is 34-67 decisions, with upper bound 68% above v1 baseline. "Comparable to v1" only holds at lower bound. |
