...
|---|---|---|---|---|
| **A** | Axis 13 medium-harness extracts a substantial fraction of v1 procedural surface | Cycle 91 (sharpened) → Cycle 96 (re-grounded post-PR#2878) | **structural argument only** (medium-harness extraction is conceptually sound; the cycle 91 "structurally validated" label rested on a 5.2× overcounted denominator and a 1.86× overcounted line-count baseline — both refuted cycle 96) | **indeterminate without verified baseline** (cycle 91 "~40-50%" / "~50-62%" claims do not survive verification; re-grounding requires unit-of-analysis decision and Phase 3 prototype measurement against verified v1 baseline of 50 step IDs / 1,289 lines) |
| **B** | Per-role decision count is mitigated below v1 baseline despite ~3-4× sub-shape adoption | Cycle 92 (sharpened) → Cycle 96 (re-grounded post-PR#2878) → Cycle 103 (counting protocol specified, closing Risk 8 at specification level) → Cycle 106 (coordination overhead decomposition specified, closing Risk 2 at specification level) | **enumeration-rubric-now-specified + coordination-decomposition-now-specified** (cycle 103 protocol uses structural-decision-points in artifacts as the primary unit, external observer enumeration, ratio-based threshold against v1 baseline measured under same rubric — replacing cycle 92's circular self-measurement; cycle 106 decomposes coordination overhead into 5 types with mechanical Type A excluded from the structural unit and Types B+C bounded as ≤8/cycle structural-invariant floor) | **ambiguous-without-prototype-but-falsifiable (hard thresholds pre-agreed: aggregate PASS ≤ 1.0× v1 / PASS-WITH-NOTE (1.0×, 1.5×] / PARTIAL-FLAG (1.5×, 2×] / REFUTED > 2×; coordination ratio ≤30% direction-validated / 30-50% direction-supported / >50% direction-refuted; reducer-rule revision rate ≤1/cycle steady-state past cycle 5 of measurement)** — cycle 92's "~80-110% of v1's" claim survives at PASS or PASS-WITH-NOTE under measurement; refuted at REFUTED. Phase 3 prototype measurement is the empirical step for both aggregate and coordination-ratio + reducer-rule-stability checks. |
