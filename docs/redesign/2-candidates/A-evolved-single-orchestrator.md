...
- **Tool migration:** ~9 new Rust crates (above). Per-crate scope claim (~200-500 LOC) empirically tested at **3 of 9 instances** (cycle 93 `v2-tool-registry` 231 prod LOC; cycle 94 `v2-cycle-history-append` 268 prod LOC; **cycle 122 `v2-phase-transition-check` 638 prod LOC — 28% above the upper bound**) — see [`Cycle 93+94+122 prototype scaffolding: migration-cost validation`](#cycle-9394122-prototype-scaffolding-migration-cost-validation) for full empirical detail. **Cycle 122 evidence widens the per-crate range claim** from "~200-500 LOC" to "~200-700 LOC for catalog/writer/validator shapes; ~500-1500 LOC predicted for orchestration-hub/retrieval shapes (unmeasured)." Aggregate net-add **~3000-5000 production LOC** under 3-crate average extrapolation (231 + 268 + 638) / 3 × 9 = ~3413, with variance widening if remaining crates' shape-mix tilts toward orchestration-hub. **Aggregate including tests: ~6000-9000 LOC** at observed 3-crate cumulative test:prod ratio of ~1.24× (refined cycle 122; cycles 93-94 had higher writer-shape ratios of 1.5×-1.9× / cycle 122 validator-shape had 0.86×). **Dependency-footprint cost is one-time at the first crate** — confirmed across 3 measurements (cycles 94 + 122 added zero new transitive deps). **Cycle 97 absorption (PR #2877 lens-4 revised range):** PR #2877's tool-surface feasibility critique revised A's aggregate net-add from 3000-4500 to **3600-6200 production LOC** based on integration/test/CI/shared-type overhead under-counting in 2-3 specific crates (`wiki-search`, `boot-phase`, `close-phase`). Cycle 122's 638-LOC observation on a non-orchestration-hub crate supports PR #2877's upward revision — if the cycle-90 per-crate range was 28% too tight for a validator-shape crate, it is likely substantially tighter for orchestration-hub shapes. Direction (per-crate boundedness) holds at 3 instances with widened range; magnitude awaits orchestration-hub or top-k retrieval crate measurement (cycle 123+ priority). PR #2877's verified workspace calibration (38 v1 crates, median 1081 LOC, mean 2118 LOC) anchors the comparison: A claims v2 crates ~5× simpler than v1 median; the bet is defensible under CORE-DESIGN-PRINCIPLE (v2 crates by-design narrower than v1's accreted multi-purpose tools), but specific crates with orchestration-hub or external-IO semantics carry the highest risk.
...
## Cycle 93+94+122 prototype scaffolding: migration-cost validation

The Migration cost section's tool-migration claims (~9 crates, ~200-500 LOC each, ~3000-4500 aggregate net-add) were named at cycle 90 authoring without empirical grounding. Cycle 93 began Phase 3 prototype scaffolding by building `v2-tool-registry`; cycle 94 extended to `v2-cycle-history-append`; cycle 122 added `v2-phase-transition-check` as the third measured crate (structurally distinct from the prior two: state-machine validation vs file write/enumerate). This section consolidates the 3-instance empirical findings. Per `direction-vs-magnitude` discipline (cycle 91 lexicon entry, HARDENED at 5+ instances): direction-validated at 3 of 9 instances; magnitude-trajectory revised per cycle 122 evidence (see below).

### Per-crate measurements (cycles 93, 94, 122)

| Metric | Cycle 93 `v2-tool-registry` | Cycle 94 `v2-cycle-history-append` | Cycle 122 `v2-phase-transition-check` |
|---|---|---|---|
| Crate type | Read-only catalog | Append-only writer | State-machine validator |
| Production LOC | ~231 | ~268 | **~638** |
| Test LOC (inline + integration) | ~347 | ~510 | ~550 |
| Total LOC | ~578 | ~804 | ~1214 |
| Test count | 15 | 31 | 43 |
| Test:Prod ratio | ~1.5× | ~1.9× | **~0.86×** |
| Build time (release, dep-cache warm) | ~1.24s | ~1.01s | ~1.17s |
| New transitive deps added to workspace lockfile | ~30 (`{clap, serde, serde_json, tempfile}` first establishment) | 0 | 0 |
| Within A's stated 200-500 LOC per-crate range | yes (lower end) | yes (lower end) | **no (28% above upper bound)** |

### Validation findings against cycle 90 authoring claims

- **Per-crate scope (~200-500 LOC) — DIRECTION-PARTIAL at 3 instances.** Cycles 93+94 fell within the stated range at the lower end (~231 / ~268 prod LOC). **Cycle 122 `phase-transition-check` came in at ~638 prod LOC — 28% above the stated upper bound and ~60% above cycle 95's pre-build estimate of "~300-400 LOC."** This is the first measured crate to exceed A's stated 200-500 range. **Honest implication:** the per-crate range claim is too narrow; validation-shape crates (multi-invariant rule engines with structured reporting) have larger surface area than file-write/enumerate crates. The cycle 90 authoring's "200-500 LOC each" was implicitly framed against simpler crate shapes. Revised per-crate range under cycle 122 evidence: **~200-700 LOC for `{read-only catalog, append-only writer, state-machine validator}` shapes, with `{orchestration hub, top-k retrieval}` shapes still unmeasured and predicted larger** (PR #2877 lens-1 absorption + cycle 93 risk #2).
- **Aggregate production-LOC claim (~3000-4500) — DIRECTION-VALIDATED; trajectory now within range.** 3-crate average: (231 + 268 + 638) / 3 = ~379 prod LOC. Extrapolation: 379 × 9 = ~3413 production LOC — within A's stated 3000-4500 range. **However:** the average is pulled up by cycle 122's higher LOC; if the remaining 6 crates' distribution resembles cycle 122 more than cycles 93-94, trajectory rises toward 4500-5000. If they resemble cycles 93-94 more, trajectory falls below 3500. The 3-instance average is **not yet stable** across crate shapes; cycle 123+ measurement of orchestration-hub or top-k-retrieval shapes is load-bearing for magnitude convergence. PR #2877's revised 3600-6200 LOC range still spans the trajectory.
- **Aggregate-with-tests claim — REVISED CYCLE 122.** Cycle 95 propagation framed aggregate-with-tests at ~7000-12000 LOC based on 1.7× average test:prod ratio. Cycle 122 measurement of 0.86× test:prod ratio REVISES this: **test:prod is highly crate-type-dependent, not monotonically widening.** 3-crate cumulative: 1407 test LOC / 1137 prod LOC = ~1.24× average. Revised aggregate-with-tests trajectory under 1.24× average: ~3413 × 2.24 = ~7645 total LOC (production + tests). The earlier 12000-LOC ceiling now appears high; ~6000-9000 total-LOC range is the cycle-122-updated estimate.
- **Dependency-footprint risk — REFUTED-AT-3 at three-crate level.** Cycle 122 added zero new transitive deps (same dep stack as cycle 94). The 0-new-deps-after-first-crate pattern now holds across 3 crates, supporting the workspace-shared-deps cutover specification.
- **Build-time aggregate — BOUNDED-AT-3.** All three crates build in ~1.0-1.2s release mode after dep cache is warm. Aggregate build time for full 9-crate set plausibly bounded at ~10-12s release (cycle 122 evidence widens the per-crate variance slightly but does not shift the aggregate ceiling materially).

### Structural insights surfaced cycles 93-94 + cycle 122
...
8. **Crate-type-dependent test:prod ratio (cycle 122).** Cycles 93+94's 1.5×-1.9× test:prod ratios were the file-write/enumerate shape. Cycle 122's validation-shape crate showed ~0.86× — substantially lower. The validation-shape crate has more production logic per-test (each invariant is tested by a few unit + one integration test); the writer-shape crate has more error paths per CLI-option per validation rule (test surface dominates). Implication: aggregate test:prod is NOT monotonically widening with crate count; it depends on the mix of crate shapes in the tool surface. Earlier "1.7× average → ~12000 total LOC ceiling" was based on writer-shape extrapolation; the validation-shape data point lowers the average and shifts the ceiling downward.

### Risks named for the cycle 93+94+122 evidence base

1. **Smaller-end bias PARTIALLY REFUTED at cycle 122; orchestration-hub/top-k-retrieval bias UNADDRESSED.** Cycle 93+94 measured the structurally-simplest crate shapes (read-only catalog + append-only writer); cycle 122 measured a validation-shape crate at 638 prod LOC, breaking the "all measured crates fall at the smaller end" claim. **However:** the 3 measured shapes (catalog / writer / validator) remain in the "non-orchestration-hub, non-external-IO" category. The remaining unmeasured crates include 2 orchestration hubs (`boot-phase`, `close-phase`) and 1 external-IO retrieval (`wiki-search`) which PR #2877 lens-1 named as the highest-risk under-estimates. Cycle 123+ measurement should prioritize these to test the under-estimate hypothesis.
2. **Test-code amplification — REVISED CYCLE 122 to "crate-shape-dependent variance."** Cycle 93's risk #1 was framed as monotonically widening; cycle 122 data point (0.86× test:prod) refutes monotonic widening but introduces a different concern: the test:prod ratio depends on crate shape. Aggregate-with-tests trajectory is bounded by the worst-case shape-mix, not by extrapolating any single ratio. Revised range: ~6000-9000 total LOC (cycle 122's 1.24× average) vs the cycle-95 stated ~7000-12000 (1.7× average). The reduction in ceiling matters for delivery-scope honesty.
3. **3-of-9 instances is direction-validated; magnitude-trajectory shifted but not stable.** Three measurements widen the evidence base from 2 to 3, but the trajectory still depends on remaining 6 crates. Cycle 123+ should prioritize crate shapes structurally distinct from the 3 already measured: orchestration-hub (`boot-phase` or `close-phase`) and top-k retrieval (`wiki-search`). These are the two crate shapes most likely to test the upper bound of per-crate LOC.
4. **Per-crate range claim too narrow.** Cycle 90 authoring's "~200-500 LOC per crate" is refuted at 1 of 3 instances (cycle 122 phase-transition-check at 638 LOC). The range needs widening to "~200-700 LOC for validation-shape and smaller; ~500-1500 LOC predicted for orchestration-hub/retrieval shape, unmeasured." This is an honest revision; aggregate trajectory updates accordingly. The cycle 95 pre-build estimate for phase-transition-check (~300-400 LOC) was 60% off; future pre-build estimates should be qualified.
...
### Sibling-pattern entries surfaced cycles 93-94 + cycle 122

- **Prototype-scaffold migration-cost-validation discipline** — TESTED-at-3 (cycles 93 + 94 + 122). Pattern: each prototype crate produces direction-vs-magnitude data; aggregation across N instances grades direction-validated vs magnitude-validated. Cycle 122 advances the discipline from 2-instance support to 3-instance with structural diversity (3 distinct crate shapes).
- **Direction-vs-magnitude discipline** — extends to 6+ instances (cycle 91 A's Axis 13 + cycle 92 B's per-role decision count + cycle 92 C's central bet + cycle 93 prototype scaffold migration-cost + cycle 94 prototype scaffold migration-cost #2 + cycle 122 prototype scaffold migration-cost #3). HARDENED.
...
- **Multi-invariant report pattern** — NOVEL at 1 instance (cycle 122). 6 invariants × {Pass, Warn, Fail, Skip} × structured details + violations list + summary + exit-code-from-status. Reusable shape for v2 validation tools (`prompt-contract-check`, `detect-abandoned-cycles`, `gardening-sweep` predicted to follow).
- **Two-layer state-tool pattern** — NOVEL at 1 instance (cycle 122). Write-time enforcement (writer crate) + read/CI-time validation (validator crate) as defense-in-depth. Cycle 94 cycle-history-append + cycle 122 phase-transition-check is the first paired instance.
- **Crate-shape-dependent test:prod ratio** — NOVEL at 1 instance (cycle 122). Direction observed: writer-shape > validator-shape in test:prod ratio. If pattern holds across more measurements, aggregate-with-tests bound depends on crate-shape mix, not on simple averaging.

### Cycle 123+ measurement plan

Cycle 122 landed `phase-transition-check` (originally cycle 96+ plan item #1). Updated plan for cycle 123+:

1. **`boot-phase`** (orchestration-hub) — first orchestration-hub measurement. PR #2877 lens-1 predicted this would exceed 500 LOC under realistic implementation (state load + cursor advance + directive checks + journal/issue writes + push semantics). Cycle 122's evidence that the per-crate range needs widening makes this measurement load-bearing for the upper-bound magnitude validation. **Cycle 121 named priority #3 first item** — load-bearing under Q7-dependent recommendation regardless of A/C selection (A is structural subset of C for shared crates).
...
3. **`prompt-contract-check`** (CI test for prompt drift) — cycle 96+ plan item #2. Likely larger than phase-transition-check (~700-1000 LOC) due to AST-style prompt parsing + contract spec parsing + pairing-event tracking (per cycle 109 extension-discipline taxonomy).
4. **`close-phase`** (orchestration-hub) — second orchestration-hub measurement, paired with `boot-phase` to test the orchestration-hub LOC variance.

Sequencing now prioritizes orchestration-hub + external-IO retrieval shapes since the 3 already-measured shapes (catalog / writer / validator) span the lower end and middle. After cycle 123+ adds 1-2 orchestration-hub measurements, the aggregate-LOC trajectory becomes magnitude-converging rather than direction-supporting.

**Cycle scope caveat:** orchestration-hub crates are predicted to exceed 75-min cycle budget for full implementation. Cycle 123+ may need to either (a) implement a minimal-viable orchestration-hub scaffold with TODO markers for deferred sub-features, with honest LOC measurement of the minimal-viable surface, or (b) split implementation across 2 cycles with the first cycle producing module structure + types + half the invariants and the second cycle completing the implementation. Option (b) is more honest under cycle 120 L2 "lower frequency higher external substance" discipline.
