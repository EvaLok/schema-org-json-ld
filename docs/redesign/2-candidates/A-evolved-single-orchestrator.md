...
- **Tool migration:** ~9 new Rust crates (above). Per-crate scope claim (~200-500 LOC) empirically tested at **4 of 9 instances** (cycle 93 `v2-tool-registry` 231 prod LOC; cycle 94 `v2-cycle-history-append` 268 prod LOC; cycle 122 `v2-phase-transition-check` 638 prod LOC — 28% above upper bound; **cycle 124 `v2-boot-phase` COMPLETE 883 prod LOC — 77% above upper bound, FIRST orchestration-hub-COMPLETE measurement, closing the cycle 123 scaffold-partial extrapolation question**) — see [`Cycle 93+94+122+124 prototype scaffolding: migration-cost validation`](#cycle-9394122124-prototype-scaffolding-migration-cost-validation) for full empirical detail. **Cycle 124 evidence supersedes cycle 123's scaffold-partial measurement** — the scaffold→complete delta was +280 prod LOC (cycle 124 added `check-standing-directives` + `identify-gardening-candidates` via `gh` CLI shell-out with fixture-injection testability), narrower than cycle 123's scaffold-extrapolation predicted (which named +300-900 LOC). **Revised per-crate range under cycle 124 evidence:** ~200-900 LOC for `{read-only catalog, append-only writer, state-machine validator, orchestration-hub-complete}` shapes; ~500-1000 LOC predicted for `{top-k retrieval}` shape (`wiki-search` still unmeasured; `close-phase` predicted in orchestration-hub-complete range based on shape-match with boot-phase). Aggregate net-add **~3600-5500 production LOC** under 4-crate average extrapolation (231 + 268 + 638 + 883) / 4 × 9 = ~4545, modestly over A's stated 4500 ceiling and within PR #2877's revised 3600-6200 range. **Aggregate including tests: ~7500-11000 LOC** at observed 4-crate cumulative test:prod ratio of ~1.18× (cycle 124 orchestration-hub-complete at 1.10×, between writer 1.5-1.9× and validator 0.86×). **Dependency-footprint cost is one-time at the first crate** — confirmed across 4 complete measurements (cycles 94 + 122 + 124 added zero new transitive deps; cycle 124 added GitHub-API integration via `gh` subprocess invocation, deliberately avoiding adding an HTTP client crate). **Cycle 97 absorption (PR #2877 lens-4 revised range):** Cycle 124's COMPLETE measurement confirms PR #2877's upward revision at the orchestration-hub shape (~883 LOC > cycle-90 200-500 range, but 41% under cycle 123's scaffold-extrapolation 1500-LOC ceiling — the prediction was 11% too high in aggregate). Direction (per-crate boundedness) holds at 4 instances with widened range; magnitude converges further as `wiki-search` and `close-phase` measurements land. PR #2877's verified workspace calibration (38 v1 crates, median 1081 LOC, mean 2118 LOC) anchors the comparison: cycle 124's 883-LOC orchestration-hub is ~82% of v1's median and ~17% of v1's `cycle-runner` (the v1 orchestration-hub-analog at ~5,200 LOC) — v2 is ~6× simpler than v1's corresponding component at the orchestration-hub level, consistent with the CORE-DESIGN-PRINCIPLE bet.
...
## Cycle 93+94+122+124 prototype scaffolding: migration-cost validation

The Migration cost section's tool-migration claims (~9 crates, ~200-500 LOC each, ~3000-4500 aggregate net-add) were named at cycle 90 authoring without empirical grounding. Cycle 93 began Phase 3 prototype scaffolding by building `v2-tool-registry`; cycle 94 extended to `v2-cycle-history-append`; cycle 122 added `v2-phase-transition-check` as the third measured crate (structurally distinct from the prior two: state-machine validation vs file write/enumerate); cycle 123 added `v2-boot-phase` as a scaffold-partial fourth crate; **cycle 124 completed `v2-boot-phase` by implementing the two deferred sub-responsibilities (`check-standing-directives` + `identify-gardening-candidates`), producing the FIRST orchestration-hub-COMPLETE measurement and closing the magnitude question cycle 123 opened.** This section consolidates the 4-instance empirical findings (4 of 9 crates measured at complete scope). Per `direction-vs-magnitude` discipline (cycle 91 lexicon entry, HARDENED at 5+ instances): direction-validated at 4 of 9 instances; magnitude-trajectory tightening (cycle 124 orchestration-hub-COMPLETE measurement came in at ~883 prod LOC — modestly below the cycle 123 scaffold-extrapolation 900-1500 prediction, indicating the scaffold-partial already contained most of the architectural complexity).

### Per-crate measurements (cycles 93, 94, 122, 124)

| Metric | Cycle 93 `v2-tool-registry` | Cycle 94 `v2-cycle-history-append` | Cycle 122 `v2-phase-transition-check` | Cycle 124 `v2-boot-phase` (COMPLETE) |
|---|---|---|---|---|
| Crate type | Read-only catalog | Append-only writer | State-machine validator | **Orchestration-hub (complete)** |
| Production LOC | ~231 | ~268 | ~638 | **~883** |
| Test LOC (inline + integration) | ~347 | ~510 | ~550 | **~971** |
| Total LOC | ~578 | ~804 | ~1214 | **~1854** |
| Test count | 15 | 31 | 43 | **44 (25 unit + 19 integration)** |
| Test:Prod ratio | ~1.5× | ~1.9× | ~0.86× | **~1.10×** |
| Build time (release, dep-cache warm) | ~1.24s | ~1.01s | ~1.17s | **~1.10s** |
| New transitive deps added to workspace lockfile | ~30 (`{clap, serde, serde_json, tempfile}` first establishment) | 0 | 0 | **0** (same dep stack across cycle 94 + 122 + 124) |
| Within A's stated 200-500 LOC per-crate range | yes (lower end) | yes (lower end) | no (28% above upper bound) | **no (77% above upper bound; 41% under cycle 123 extrapolation 1500 ceiling)** |
| Scope completeness | complete | complete | complete | **complete (5 of 5 sub-responsibilities; cycle 123 scaffold-partial extended cycle 124)** |
...
- **Per-crate scope (~200-500 LOC) — DIRECTION-PARTIAL at 4 instances; 2 of 4 above the stated range; orchestration-hub-COMPLETE measurement closes the cycle 123 scaffold-partial extrapolation question.** Cycles 93+94 fell within the stated range at the lower end (~231 / ~268 prod LOC). Cycle 122 `phase-transition-check` at ~638 prod LOC (28% above upper bound). **Cycle 124 `boot-phase` COMPLETE at ~883 prod LOC — 77% above the upper bound, but 41% UNDER the cycle 123 scaffold-extrapolation 1500-LOC ceiling.** The +280 LOC delta from scaffold-partial (603) to complete (883) is narrower than the cycle 123 prediction of +300-900 LOC, indicating that the multi-stage report pattern (cycle 123 NOVEL@1) absorbed most of the architectural complexity at scaffold scope. **Revised per-crate range under cycle 124 evidence:** ~200-900 LOC for `{read-only catalog, append-only writer, state-machine validator, orchestration-hub-complete}` shapes; ~500-1000 LOC predicted for `{top-k retrieval}` shape (`wiki-search` still unmeasured; `close-phase` predicted in the orchestration-hub-complete range based on shape-match with boot-phase).
- **Aggregate production-LOC claim (~3000-4500) — DIRECTION-VALIDATED; trajectory now centered in upper half of stated range, not exceeded.** 4-crate average: (231 + 268 + 638 + 883) / 4 = ~505 prod LOC. Extrapolation: 505 × 9 = ~4545 production LOC — modestly over A's stated 4500 upper bound, within PR #2877's revised 3600-6200 range. **Cycle 124 finding (vs cycle 123 prediction):** the cycle 123 scaffold-extrapolation predicted 9-crate aggregate of ~5030 (using a hypothetical 1100 LOC complete boot-phase); actual cycle 124 yields ~4545 — closer to A's stated 4500 ceiling, less of an exceedance than cycle 123 framing suggested. The cycle 123 prediction was 11% too high; the orchestration-hub-shape is at the high end of A's range, not above it by a wide margin.
- **Aggregate-with-tests claim — REFINED CYCLE 124.** Cycle 123 used 4-crate cumulative test:prod ratio of 1.16× to project 7500-11500 LOC. Cycle 124's orchestration-hub-COMPLETE has 1.10× test:prod (971/883), modestly above the cycle 123 scaffold-partial 1.02× ratio. New 4-crate cumulative: (347+510+550+971) / (231+268+638+883) = 2378 / 2020 = **~1.18× average**. Revised trajectory: ~4545 × (1 + 1.18) = **~9908 total LOC** for the 9-crate set under the new cumulative ratio. **Revised range: ~7500-11000 total LOC** (tighter than cycle 123's 7500-11500 because the orchestration-hub-COMPLETE measurement reduces the variance contributed by the previously-extrapolated boot-phase delta).
- **Dependency-footprint risk — REFUTED-AT-4 (firmly) at four-crate-complete level.** Cycle 124 added zero new transitive deps. Same `{clap, serde, serde_json, tempfile}` stack as cycles 94 + 122 + 123. The 0-new-deps-after-first-crate pattern holds across 4 crates × 4 distinct shapes × scaffold-partial-vs-complete extension. The workspace-shared-deps cutover specification is robustly supported.
- **Build-time aggregate — BOUNDED-AT-4.** All four crates build in ~1.0-1.2s release mode after dep cache is warm. Cycle 124's 1.10s build is virtually identical to cycle 123's scaffold (1.09s), confirming that adding GitHub-API integration via `gh` shell-out + JSON parsing does NOT meaningfully widen build time (no new compile-time deps introduced; the integration is via subprocess invocation rather than a Rust HTTP client library).
