...
- **Tool migration:** ~9 new Rust crates (above). Per-crate scope claim (~200-500 LOC) empirically tested at **4 of 9 instances** (cycle 93 `v2-tool-registry` 231 prod LOC; cycle 94 `v2-cycle-history-append` 268 prod LOC; cycle 122 `v2-phase-transition-check` 638 prod LOC — 28% above upper bound; **cycle 123 `v2-boot-phase` scaffold 603 prod LOC — 21% above upper bound, first orchestration-hub-shape measurement at scaffold-partial scope**) — see [`Cycle 93+94+122+123 prototype scaffolding: migration-cost validation`](#cycle-9394122123-prototype-scaffolding-migration-cost-validation) for full empirical detail. **Cycle 123 evidence is a SCAFFOLD-PARTIAL data point** (3 of 5 sub-responsibilities implemented: `state-load` + `cursor-advance` + `cycle-context.json` emission; 2 deferred: `standing-directive-check` + `gardening-sweep-pre-cycle`). The complete `boot-phase` is predicted to land in the 900-1500 LOC range when cycle 124+ implements the deferred sub-responsibilities — supporting PR #2877 lens-1's under-estimate prediction at the orchestration-hub shape. **Cycle 123 widens the per-crate range claim** to "~200-700 LOC for catalog/writer/validator/orchestration-hub-scaffold shapes; ~900-1500 LOC predicted for complete orchestration-hub + top-k retrieval shapes (still unmeasured at completion)." Aggregate net-add **~3500-5500 production LOC** under 4-crate average extrapolation (231 + 268 + 638 + 603) / 4 × 9 = ~3915, with variance widening upward when remaining unmeasured crates land at the orchestration-hub or retrieval shape. **Aggregate including tests: ~7500-11500 LOC** at observed 4-crate cumulative test:prod ratio of ~1.16× (cycles 93-94 writer-shape 1.5×-1.9× / cycle 122 validator-shape 0.86× / cycle 123 orchestration-hub-scaffold 1.02× — ratio is highly crate-shape-dependent). **Dependency-footprint cost is one-time at the first crate** — confirmed across 4 measurements (cycles 94 + 122 + 123 added zero new transitive deps). **Cycle 97 absorption (PR #2877 lens-4 revised range):** PR #2877's tool-surface feasibility critique revised A's aggregate net-add from 3000-4500 to **3600-6200 production LOC** based on integration/test/CI/shared-type overhead under-counting in 2-3 specific crates (`wiki-search`, `boot-phase`, `close-phase`). Cycle 123's 603-LOC SCAFFOLD measurement on the first orchestration-hub crate empirically supports PR #2877's upward revision — the orchestration-hub shape exceeds the cycle-90 200-500 range even at minimal-viable scaffold scope, and the complete implementation will substantially exceed it. Direction (per-crate boundedness) holds at 4 instances with widened range; magnitude approaches convergence as cycle 124+ orchestration-hub completion + `wiki-search` measurement land. PR #2877's verified workspace calibration (38 v1 crates, median 1081 LOC, mean 2118 LOC) anchors the comparison: A claims v2 crates ~5× simpler than v1 median; the bet is defensible under CORE-DESIGN-PRINCIPLE (v2 crates by-design narrower than v1's accreted multi-purpose tools), but specific crates with orchestration-hub or external-IO semantics carry the highest risk — cycle 123 evidence confirms this at the scaffold-partial level.
...
## Cycle 93+94+122+123 prototype scaffolding: migration-cost validation

The Migration cost section's tool-migration claims (~9 crates, ~200-500 LOC each, ~3000-4500 aggregate net-add) were named at cycle 90 authoring without empirical grounding. Cycle 93 began Phase 3 prototype scaffolding by building `v2-tool-registry`; cycle 94 extended to `v2-cycle-history-append`; cycle 122 added `v2-phase-transition-check` as the third measured crate (structurally distinct from the prior two: state-machine validation vs file write/enumerate); cycle 123 added `v2-boot-phase` as the **fourth measured crate and first orchestration-hub-shape measurement** (scaffold-partial — `state-load` + `cursor-advance` + `cycle-context.json` emission implemented; `standing-directive-check` + `gardening-sweep-pre-cycle` explicitly Deferred per Option B from cycle 122 _notes). This section consolidates the 4-instance empirical findings. Per `direction-vs-magnitude` discipline (cycle 91 lexicon entry, HARDENED at 5+ instances): direction-validated at 4 of 9 instances; magnitude-trajectory now under partial orchestration-hub-shape evidence (full validation awaits cycle 124+ completion of boot-phase + measurement of `wiki-search` / `close-phase`).

### Per-crate measurements (cycles 93, 94, 122, 123)

| Metric | Cycle 93 `v2-tool-registry` | Cycle 94 `v2-cycle-history-append` | Cycle 122 `v2-phase-transition-check` | Cycle 123 `v2-boot-phase` (scaffold-partial) |
|---|---|---|---|---|
| Crate type | Read-only catalog | Append-only writer | State-machine validator | **Orchestration-hub (scaffold)** |
| Production LOC | ~231 | ~268 | ~638 | **~603** |
| Test LOC (inline + integration) | ~347 | ~510 | ~550 | ~617 |
| Total LOC | ~578 | ~804 | ~1214 | ~1220 |
| Test count | 15 | 31 | 43 | **33** |
| Test:Prod ratio | ~1.5× | ~1.9× | ~0.86× | **~1.02×** |
| Build time (release, dep-cache warm) | ~1.24s | ~1.01s | ~1.17s | ~1.09s |
| New transitive deps added to workspace lockfile | ~30 (`{clap, serde, serde_json, tempfile}` first establishment) | 0 | 0 | **0** |
| Within A's stated 200-500 LOC per-crate range | yes (lower end) | yes (lower end) | no (28% above upper bound) | **no (21% above upper bound)** |
| Scope completeness | complete | complete | complete | **scaffold-partial (3 of 5 sub-responsibilities; 2 explicitly Deferred)** |

### Validation findings against cycle 90 authoring claims

- **Per-crate scope (~200-500 LOC) — DIRECTION-PARTIAL at 4 instances; 2 of 4 above the stated range.** Cycles 93+94 fell within the stated range at the lower end (~231 / ~268 prod LOC). Cycle 122 `phase-transition-check` came in at ~638 prod LOC — 28% above the stated upper bound. **Cycle 123 `boot-phase` scaffold-partial came in at ~603 prod LOC — 21% above the upper bound, the first orchestration-hub-shape measurement (at minimal-viable scaffold scope; 2 of 5 sub-responsibilities Deferred).** This is the second consecutive measurement to exceed A's stated 200-500 range, now at 50% of measured crates. **Honest implication:** the per-crate range claim is too narrow; multi-stage / multi-invariant rule engines and orchestration-hub coordinators have larger surface area than file-write/enumerate crates. **The cycle 123 measurement carries an additional implication beyond cycle 122's: the orchestration-hub shape exceeds 500 LOC even at scaffold-partial scope, which means PR #2877 lens-1's under-estimate prediction is supported BEFORE the complete orchestration-hub implementation lands.** The complete `boot-phase` (with `standing-directive-check` + `gardening-sweep-pre-cycle`) is predicted to fall in the 900-1500 LOC range. Revised per-crate range under cycle 123 evidence: **~200-700 LOC for `{read-only catalog, append-only writer, state-machine validator, orchestration-hub-scaffold}` shapes; ~900-1500 LOC predicted for `{complete orchestration-hub, top-k retrieval}` shapes (top-k still unmeasured; orchestration-hub-complete pending cycle 124+).**
- **Aggregate production-LOC claim (~3000-4500) — DIRECTION-VALIDATED; trajectory now in upper half of stated range.** 4-crate average: (231 + 268 + 638 + 603) / 4 = ~435 prod LOC. Extrapolation: 435 × 9 = ~3915 production LOC — in the upper half of A's stated 3000-4500 range. **Cycle 123 caveat:** boot-phase is scaffold-partial; the complete implementation will land cycle 124+ and push the average upward. If the complete boot-phase comes in at ~1100 LOC (mid-range of the 900-1500 prediction), the 4-crate average shifts to (231+268+638+1100)/4 = ~559 prod LOC, with 9-crate extrapolation of ~5030 — exceeding A's stated 4500 upper bound. **The cycle 123 data thus refines the cycle 122 framing: A's aggregate claim is at risk of upper-bound exceedance once orchestration-hub completion lands.** PR #2877's revised 3600-6200 LOC range still spans the projected trajectory.
- **Aggregate-with-tests claim — REFINED CYCLE 123.** Cycle 122 revised the aggregate-with-tests ceiling downward to ~6000-9000 LOC using 1.24× cumulative test:prod ratio. Cycle 123's orchestration-hub scaffold has 1.02× test:prod ratio (617 / 603), pulling the 4-crate cumulative ratio to: (347+510+550+617) / (231+268+638+603) = 2024 / 1740 = **~1.16× average**. The trajectory remains highly crate-shape-dependent (writer-shape 1.5-1.9× / validator-shape 0.86× / orchestration-hub-scaffold 1.02×). Revised aggregate-with-tests trajectory under 1.16× average: ~3915 × 2.16 = **~8460 total LOC (production + tests)**. If complete boot-phase shifts the prod average to ~559, total trajectory becomes ~559 × 9 × (1 + 1.16) = ~10,870 — pushing back toward the cycle 95 12000-LOC ceiling. **Revised range: ~7500-11500 total LOC for the 9-crate set.**
- **Dependency-footprint risk — REFUTED-AT-4 at four-crate level.** Cycle 123 added zero new transitive deps (same dep stack as cycles 94 + 122). The 0-new-deps-after-first-crate pattern now holds across 4 crates spanning 4 distinct shapes. The workspace-shared-deps cutover specification is strongly supported.
- **Build-time aggregate — BOUNDED-AT-4.** All four crates build in ~1.0-1.2s release mode after dep cache is warm. Aggregate build time for full 9-crate set plausibly bounded at ~10-12s release. Cycle 123's 1.09s build is in line with cycle 122; the orchestration-hub-shape does not introduce additional build overhead at the scaffold level.
...
### Risks named for the cycle 93+94+122+123 evidence base

1. **Smaller-end bias REFUTED at cycle 123; orchestration-hub-COMPLETE bias UNADDRESSED.** Cycle 93+94 measured the structurally-simplest crate shapes (read-only catalog + append-only writer); cycle 122 measured a validation-shape crate at 638 prod LOC; **cycle 123 measured an orchestration-hub-shape SCAFFOLD at 603 prod LOC** — breaking the "all measured crates fall at the smaller end" claim definitively. **However:** cycle 123 boot-phase is scaffold-partial. The complete orchestration-hub (boot-phase + standing-directive-check + gardening-sweep) is still unmeasured; the cycle 95 prediction was ~300-400 LOC for the COMPLETE crate, but cycle 123 shows ~600 LOC for ~60% of the surface. Extrapolated complete: ~900-1500 LOC. PR #2877 lens-1's under-estimate prediction is supported at scaffold scope and likely strengthened at completion scope (cycle 124+).
2. **Test-code amplification — REVISED CYCLE 123 to "crate-shape-dependent + completeness-dependent."** Cycle 122 named "crate-shape-dependent variance"; cycle 123 refines this with a third dimension: completeness. The scaffold-partial measurement has its own test:prod profile (1.02× — between writer 1.5-1.9× and validator 0.86×). Aggregate-with-tests trajectory under cycle 123 4-crate cumulative ratio of 1.16× lands at ~7500-11500 total LOC for the 9-crate set. The reduction from cycle 95's 7000-12000 stays valid in direction but the upper bound creeps back upward as the orchestration-hub completion will likely shift the cumulative ratio.
3. **4-of-9 instances: direction-validated; magnitude-trajectory narrowing.** Four measurements span the simplest to mid-complex shapes; the remaining unmeasured crates (`close-phase`, `wiki-search`, `gardening-sweep`, `detect-abandoned-cycles`, `prompt-contract-check`) are predicted to span the upper half of the range. The 4-crate average (~435 LOC) is biased slightly low because boot-phase is scaffold-partial; the unbiased estimate (assuming boot-phase completes at ~1100 LOC) is (231+268+638+1100)/4 = ~559 prod LOC, suggesting the 9-crate aggregate is in the 4500-5500 range — above A's stated 4500 upper bound and within PR #2877's revised 3600-6200 range.
4. **Per-crate range claim too narrow — REFUTED at 50% of measurements.** Cycle 90 authoring's "~200-500 LOC per crate" is refuted at 2 of 4 instances (cycle 122 phase-transition-check 638 LOC; cycle 123 boot-phase scaffold 603 LOC). The range needs widening to **"~200-700 LOC for catalog/writer/validator/orchestration-hub-scaffold shapes; ~900-1500 LOC predicted for complete orchestration-hub + top-k retrieval shapes."** This is an honest revision; aggregate trajectory updates accordingly. The cycle 95 pre-build estimate for phase-transition-check (~300-400 LOC) was 60% off; cycle 123 had no pre-build LOC estimate for boot-phase scaffold (deliberately omitted given cycle 122's surprise); future pre-build estimates should be qualified with shape + completeness expectations.
...
8. **Specific-crate under-estimate risk (added cycle 97 per PR #2877 lens-1 absorption; STRENGTHENED CYCLE 123).** PR #2877's per-crate critique names 3 crates as likely under-estimated: `boot-phase` and `close-phase` (orchestration hubs touching state load + cursor advance + directive checks + journal/issue writes + push semantics — 200-500 LOC optimistic with robust error paths and tests); `wiki-search` (top-k retrieval over `_notes/*.md` description-frontmatter — practical load includes indexing, update strategy, ranking semantics, corruption handling, query contract — likely >500 LOC). **Cycle 123 empirically supports the boot-phase under-estimate prediction at scaffold-partial scope** — the minimal-viable orchestration-hub already exceeds the cycle-90 range. The COMPLETE boot-phase will further confirm this. Counterbalanced: `phase-transition-check`, `tool-registry`, `cycle-history-append` are over-estimated at upper bound (cycles 93+94 measurements at 231 / 268 LOC fall at lower end; cycle 122 phase-transition-check at 638 was modest over-shoot). Net implication: aggregate trajectory shifts upward toward PR #2877's revised 3600-6200 LOC range, with cycle 123 evidence pushing closer to the upper half (~5000+ LOC).
...
### Sibling-pattern entries surfaced cycles 93-94 + cycle 122 + cycle 123

- **Prototype-scaffold migration-cost-validation discipline** — TESTED-at-4 (cycles 93 + 94 + 122 + 123). Pattern: each prototype crate produces direction-vs-magnitude data; aggregation across N instances grades direction-validated vs magnitude-validated. Cycle 123 advances the discipline from 3-instance support to 4-instance with the first orchestration-hub-shape data point, at scaffold-partial scope (with explicit Deferred contract markers).
