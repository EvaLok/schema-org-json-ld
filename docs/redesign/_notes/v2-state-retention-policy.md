...
| Threshold | Value (cycle 167 recalibration) | Rationale |
|---|---|---|
| Advisory | 150 dispatches retained live | Above realistic V2 cadence; investigate before drift accumulates. Post-archival baseline ~70 + ~30-day window of moderately-active V2 dispatch cadence leaves comfortable headroom at 150. |
| Mandatory | 400 dispatches; archive via `v2-state-dispatch-archive` | Sustained heavy dispatch + delayed archival; trigger archival sweep. Above this implies archival has not run for several months OR dispatch cadence has spiked to V1-era rates. |
| Hard | 1000 dispatches | Refuse-to-write. Above this is structurally broken — either archival is failing or there is a write-leak. V1 reached ~930 over 3 months without archival; with archival operational, no normal V2 trajectory reaches this. |

**Owners:** `v2-state-dispatch-sync` writes status mutations (extension role; cycle 154); `v2-state-dispatch-archive` performs the archival sweep (cycle 163-165 design + impl; cycle 166 first live invocation reduced 931→69). Hard-threshold refuse-to-write is named here but not yet implemented in either writer; forward work.
...
**Storage-key clarification (cycle 159):** the conceptual "dispatches array" is stored under the key `agent_sessions` in `docs/state.json` (legacy v1 naming). The `v2-state-audit` tool reads this key directly. Cycle 158 policy authoring used the conceptual name "dispatches" without checking the implementation; cycle 159 tool-extraction surfaced the gap. A future v2 migration may rename the key; tool + policy revise together when that happens. Cycle 159 live audit reported 930 entries against the then-500-entry hard threshold — the audit tool correctly classified this as `hard` with `halt-session-start` recommendation. The hard-threshold semantics are owner-enforced (the writer refuses), not audit-tool-enforced; the audit tool only reports. The 930-entry state was observable-but-tolerated because no writer implements refuse-to-write. Cycle 166 archival sweep brought the count to 69 entries (4 live + 65 terminal-but-within-30-day-window); the recalibration in the table above is sized against this new operational baseline.

**Cycle 167 recalibration rationale (this section).** Initial thresholds (cycle 158: 50 / 200 / 500) assumed a slow steady-state stream without an archival mechanism. Late-V1 ran at ~10 dispatches/cycle and accumulated 930 entries over 3 months; cycle 159 measurement made the assumption-vs-reality gap explicit. Cycle 163 designed `v2-state-dispatch-archive` (30-day age threshold sweep to dated archive files); cycle 164-165 dispatch+merge landed the implementation; cycle 166 Track 1 first-live-invocation reduced 931→69 (~93% reduction) and surfaced the new operational reality. Empirical V2 dispatch cadence over the cycle 138-167 window: 2 dispatches in 30 cycles (~0.07/cycle; cycles 164 #2974 + 166 #2978). Eva directive #2937 explicitly authorizes higher cadence, so projection ranges: low ~0.27/day (current actual), medium ~1/day, active ~2/day, heavy ~5/day. 30-day window steady-state at those rates: 70 + 8 / 70 + 30 / 70 + 60 / 70 + 150 = 78 / 100 / 130 / 220 entries. Recalibrated thresholds sit ABOVE these projections so they signal anomaly, not normal V2 operation:
- **Advisory 150** ≈ heavy cadence ceiling (5/day for 30 days) + safety margin. Below this is normal operation.
- **Mandatory 400** ≈ ~2× advisory; sustained excess of heavy cadence OR archival not running for ~2 months at active cadence.
- **Hard 1000** ≈ V1-era accumulation rate without archival running for ~3+ months. Reaching this indicates a write-leak or archival breakage.

Forward work preserved from cycle 158/159: implement refuse-to-write in writers (named: `v2-state-dispatch-sync` and `v2-state-dispatch-archive` both need the bound check at write-finalize). The audit tool (`v2-state-audit`) continues to compare against these table values and emit advisory/mandatory/hard kinds; the runner halt-on-hard (cycle 162 pre-flight wiring at §7 below) remains the operational backstop until writer-side refuse-to-write lands.
