...
**Storage-key clarification (cycle 159):** the conceptual "dispatches array" is stored under the key `agent_sessions` in `docs/state.json` (legacy v1 naming). The `v2-state-audit` tool reads this key directly. Cycle 158 policy authoring used the conceptual name "dispatches" without checking the implementation; cycle 159 tool-extraction surfaced the gap. A future v2 migration may rename the key; tool + policy revise together when that happens. Cycle 159 live audit reports 930 entries against the 500-entry hard threshold — the audit tool correctly classifies this as `hard` with `halt-session-start` recommendation. The hard-threshold semantics are owner-enforced (v2-state-dispatch-sync would refuse the write), not audit-tool-enforced; the audit tool only reports. Since v2-state-dispatch-sync does not yet implement refuse-to-write, the 930-entry state is observable-but-tolerated. Forward work: (a) implement refuse-to-write in v2-state-dispatch-sync OR (b) calibrate the threshold against historical growth (3-month accumulation from 2026-02-24 produced 930 entries; ~10/cycle in late-V1, much higher than cycle 158's "50 advisory" estimate which assumed a slower rate).
