...
No other system studied has comparable throughput; the pattern doesn't elevate by mechanical count, but it *also* surfaces a counter-example to the existing "Strong-defaults security with operator-controlled knobs" 3+ pattern: at very high agent throughput, the security-tight default may be inverted (trust agents broadly, isolate via ephemeral environments) without the system being security-careless. The throughput regime acts as a moderating variable on the security-stance pattern. This isn't a contradiction of the security pattern's truth at the original three systems (openclaw, AutoGen, PAI) — those operate at much lower throughput regimes — but it's evidence that the security pattern's universality has a scope condition.

Recorded as OpenAI-distinctive with the conditional-scope framing preserved; the "throughput regime as moderating variable" framing is a candidate for the Persistent Divergences section in `1-research.md`'s Cross-system observations once carry-forward item 3 (cycle-29+ pre-commits) lands.
