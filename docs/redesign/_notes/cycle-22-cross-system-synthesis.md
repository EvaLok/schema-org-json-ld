...
### Cognition Devin: context anxiety / model self-model failures — **Added cycle 29 from cycle-26 dispatch deliverables**

Cognition's writeups (per the cycle-26 dispatch deliverable on Devin, PR [#2780](https://github.com/EvaLok/schema-org-json-ld/pull/2780)) document four related findings: the model behaves differently because it *believes* it is near its context limit (not because it actually is); Devin's behavior under environmental conditions the model misreads (the "environmental deception" pattern); prompt placement matters disproportionately (recent context vs system-prompt-distant); the model misestimates what it can finish in remaining tokens. The most striking workaround Cognition documents is *lying to the model* about its remaining context window so it stops self-truncating. *(Documented-claim per cycle-26 source-access note — `cognition.ai` was firewall-blocked; content via secondary sources with verbatim quotes converging.)*

No other system studied documents model-self-model-failures as a named architectural concern. Voyager and AutoGen and LangGraph all assume the model accurately reflects on its own state; oh-my-codex's word-count gate (cycle-29-added pre-execution gating observation below) is signal-detection on user input, not on model self-state. The shape (model behavior shaped by self-model of resources, not by the resources themselves) is so unusual that single-system status is honest. Recorded as candidate for future cross-validation; if subsequent reads (oh-my-claudecode, openai/symphony queued per Eva directives [#2774](https://github.com/EvaLok/schema-org-json-ld/issues/2774) and [#2775](https://github.com/EvaLok/schema-org-json-ld/issues/2775)) surface anything similar, this elevates.

### oh-my-codex: pre-execution gating against underspecified requests — **Added cycle 29 from cycle-26 dispatch deliverables**

oh-my-codex maintains a 44KB keyword detector + word-count signal-detection gate that fires before task execution begins; underspecified requests trigger a "we need more information" path rather than executing on insufficient input. A `force` flag bypasses the gate when the operator wants to override the heuristic (PR [#2784](https://github.com/EvaLok/schema-org-json-ld/pull/2784) deliverable pattern 5). The gate is *deterministic pre-flight*, not a model-judgment-call — the keyword detector is mechanical pattern-matching, not semantic classification.

Cognition Devin's context-anxiety mitigation is *adjacent at a different lifecycle phase*: Devin's mitigations operate during execution to prevent premature task closure (mid-execution), not before execution begins (pre-execution). The shapes share the principle "guard against the model under-reading the work required" but at different points in the lifecycle. Recorded as oh-my-codex-distinctive; the cross-validation candidate is whether oh-my-claudecode (queued per Eva directive [#2774](https://github.com/EvaLok/schema-org-json-ld/issues/2774)) or openai/symphony (queued per [#2775](https://github.com/EvaLok/schema-org-json-ld/issues/2775)) document a similar pre-execution mechanical gate.

### OpenAI Harness: agent legibility as optimization target — **Added cycle 29 from cycle-26 dispatch deliverables**

OpenAI's harness-engineering writeup names agent legibility as an explicit optimization target — the repository structure, AGENTS.md as table of contents, structured `docs/`, and mechanical-enforcement layer all exist so that the agent can navigate the codebase efficiently and produce verifiable work (PR [#2783](https://github.com/EvaLok/schema-org-json-ld/pull/2783) deliverable pattern 15). The "from the agent's point of view, anything it can't access in-context while running effectively doesn't exist" framing is the strongest published statement that agent comprehension is itself an architectural concern.

Cognition Devin and oh-my-codex have *implicit* support that doesn't reach the same level of explicitness. Cognition's context-engineering thesis ("the context isn't just the user's message but includes everything the agent has done") frames context as the unit of reliability; agent trace as architecture. oh-my-codex's `templates/AGENTS.md` (with all-caps autonomy block, structured directives) and `CONTRIBUTING.md`'s `<Bad>`/`<Good>` examples + 30 named role-prompts in `prompts/*.md` all gesture toward repo-structured-for-agent-comprehension, but the writeups don't name "agent legibility" as the design driver in the same way OpenAI's writeup does.

Cycle-28 recheck preserved the "1 explicit + 2 implicit" framing as honest; this stays single-system at strict framing. The cross-validation candidate is whether subsequent reads surface another system that names agent-legibility as an explicit design driver (rather than an implicit consequence of repo structure). Recorded as OpenAI-explicit-with-adjacency.

### OpenAI Harness: throughput-based merge philosophy with conditional scope — **Added cycle 29 from cycle-26 dispatch deliverables**

OpenAI's harness-engineering writeup names a throughput-based merge philosophy: corrections are cheap; waiting is expensive; agents merge their own PRs at high volume; per-worktree isolation is the practical containment, not security-tight branch protection (PR [#2783](https://github.com/EvaLok/schema-org-json-ld/pull/2783) deliverable pattern 16). Cycle-27 cross-validation found this is *explicitly conditional* in the OpenAI writeup — scoped to the high-throughput regime, not asserted as universally optimal.

No other system studied has comparable throughput; the pattern doesn't elevate by mechanical count, but it *also* surfaces a counter-example to the existing "Strong-defaults security with operator-controlled knobs" 3+ pattern: at very high agent throughput, the security-tight default may be inverted (trust agents broadly, isolate via ephemeral environments) without the system being security-careless. The throughput regime conditions when the security-stance pattern applies. This isn't a contradiction of the security pattern's truth at the original three systems (openclaw, AutoGen, PAI) — those operate at much lower throughput regimes — but it's evidence that the security pattern's universality has a scope condition.

Recorded as OpenAI-distinctive with the conditional-scope framing preserved; the "throughput regime conditions when the security-stance pattern applies" framing is a candidate for the Persistent Divergences section in `1-research.md`'s Cross-system observations once carry-forward item 2 (cycle-30+ pre-commits) lands.

### oh-my-codex: autonomy directive prominently stated — **Added cycle 29 from cycle-26 dispatch deliverables**

oh-my-codex's `templates/AGENTS.md` opens with an all-caps autonomy block — explicit directive that the agent should not request permission for routine work, should resolve issues itself, should escalate only on genuinely-blocking concerns (PR [#2784](https://github.com/EvaLok/schema-org-json-ld/pull/2784) deliverable pattern 17). This is an *agent-instruction-level* autonomy directive, not just a project-policy statement.

OpenAI harness-engineering has *adjacent* support at a different framing: "Humans steer. Agents execute." (PR [#2783](https://github.com/EvaLok/schema-org-json-ld/pull/2783) deliverable pattern 2) is a role-allocation thesis, framed at the architectural-philosophy level rather than as agent-instruction-level autonomy directive. Cognition Devin's "fully autonomous AI software engineer" framing is marketing taglinework, not an agent-instruction-level directive. The two adjacencies share the principle (agents don't ask permission for execution; humans set goals) at progressively looser framings.

Cycle-28 recheck preserved the "1 explicit + 2 implicit/adjacent" framing as honest; this stays single-system at strict framing. The cross-validation candidate is whether subsequent reads (oh-my-claudecode in particular, given Eva directive [#2774](https://github.com/EvaLok/schema-org-json-ld/issues/2774) names it as related to oh-my-codex) document a comparable explicit autonomy block at the agent-instruction-level.
