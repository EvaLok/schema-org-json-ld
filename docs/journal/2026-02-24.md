...
Both agent dispatches from the first cycle ([#10](https://github.com/EvaLok/schema-org-json-ld/issues/10) AggregateRating, [#11](https://github.com/EvaLok/schema-org-json-ld/issues/11) Review/Rating) failed with:
...
Eva confirmed in [issue #13](https://github.com/EvaLok/schema-org-json-ld/issues/13) that she updated the PAT with read/write for actions, contents, issues, and pull requests. Closed [#13](https://github.com/EvaLok/schema-org-json-ld/issues/13).

### First successful dispatches

- **[Issue #15](https://github.com/EvaLok/schema-org-json-ld/issues/15) → [PR #16](https://github.com/EvaLok/schema-org-json-ld/issues/16)**: AggregateRating (copilot_work_started at 16:08:16Z)
- **[Issue #17](https://github.com/EvaLok/schema-org-json-ld/issues/17) → [PR #18](https://github.com/EvaLok/schema-org-json-ld/issues/18)**: Review + Rating (copilot_work_started at 16:08:31Z)
...
Issue numbers and PR numbers share a namespace on GitHub. Creating [issue #15](https://github.com/EvaLok/schema-org-json-ld/issues/15) resulted in [PR #16](https://github.com/EvaLok/schema-org-json-ld/issues/16) (the next available number). Creating [issue #17](https://github.com/EvaLok/schema-org-json-ld/issues/17) resulted in [PR #18](https://github.com/EvaLok/schema-org-json-ld/issues/18). This is expected but worth noting — when mapping issues to PRs, the PR number is always the issue number + 1 in practice (assuming no other activity in between).
...
[Issue #19](https://github.com/EvaLok/schema-org-json-ld/issues/19) from Eva: the `copilot-setup-steps.yml` file pre-installs PHP 8.3 and composer deps before the agent starts. This means agents can now self-verify by running `composer run test-unit`. Updated AGENTS.md with a "Running Tests" section making this explicit.
...
- [PR #22](https://github.com/EvaLok/schema-org-json-ld/issues/22) (Organization): Used tabs correctly, matching codebase style
- [PR #24](https://github.com/EvaLok/schema-org-json-ld/issues/24) (FAQPage): No indentation at all in class/method bodies
...
Successfully used `@copilot` on [PR #24](https://github.com/EvaLok/schema-org-json-ld/issues/24) to request indentation fix. The revision session took ~5 minutes. The agent understood the feedback, fixed all 6 files, and tests still passed. This validates the revision workflow.
...
Eva requested tools for two repetitive operations ([#40](https://github.com/EvaLok/schema-org-json-ld/issues/40), [#42](https://github.com/EvaLok/schema-org-json-ld/issues/42)). Created three shell scripts in `tools/`:
...
**Context**: Seventh cycle. Clean slate. One Eva input ([#52](https://github.com/EvaLok/schema-org-json-ld/issues/52)) about PR review workflow.

### New PR review workflow works well

Eva's input [#52](https://github.com/EvaLok/schema-org-json-ld/issues/52) was actionable: CI workflows only run on non-draft PRs, so the previous approach of reviewing while still draft was incorrect. Created a skill doc, a `tools/review-pr` script, and updated the checklist.
