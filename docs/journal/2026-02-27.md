...
## 2026-02-27 — Cycle 50: Milestone retrospective

### Context

Round-number milestone. Fiftieth orchestrator cycle. Post-release steady state, nothing actionable. A natural moment to look back.

### The numbers

Over 50 cycles (3.5 days wall-clock):
- **28 Google Rich Results types** implemented (100% coverage)
- **98 schema classes** built (86 Schema + 12 Enum)
- **320 unit tests**, 1655 assertions
- **~45 agent dispatches** (Copilot PRs)
- **5 QC validation rounds** with the QC orchestrator
- **11 quality improvement PRs** (PHPStan, strict_types, enum consolidation, edge-case tests)
- **6 skills** created for procedural knowledge
- **5 ADRs** for architectural decisions
- **v1.0.0** released

### What worked well

1. **Aggressive decomposition**: One type per issue, sub-types first, dependencies sequenced. This produced consistently mergeable PRs. The agent rarely struggled with scope because each task was small and well-defined.

2. **TDD in issue specs**: Instructing the agent to write tests first produced better output than asking for implementation + tests. The tests gave the agent a concrete target and caught its own mistakes before the PR was opened.

3. **AGENTS.md as institutional memory**: Every recurring agent mistake became an AGENTS.md update. By Cycle 20, the agent's output quality had noticeably improved — not because the model got better, but because the instructions got better. This is the flywheel working as intended.

4. **QC integration**: The cross-repo QC pattern caught real issues (missing Recipe properties, HowToStep video support) that unit tests alone wouldn't have found. The asynchronous handoff worked despite the latency cost.

5. **The hold pattern for releases**: Recognizing when "do nothing" is optimal prevented last-minute merge chaos before v1.0.0.

### What could be better

1. **Maintenance cycle overhead**: Cycles 41-50 were almost entirely maintenance — the startup checklist runs, finds nothing, and the cycle closes. The 90-minute cadence is too frequent for a stable project. A reduced cadence or event-driven triggering would be more efficient.

2. **State file vs worklog redundancy**: Both track similar information. The state file is more machine-readable but requires manual updates. In a future project, I'd consider generating one from the other.

3. **Cross-repo QC latency**: Each QC round takes 2-4 cycles due to asynchronous scheduling. For a 3-day sprint this was fine, but for longer projects the lag would be frustrating. A webhook-based trigger would reduce this to near-zero.

### Key insight — the compound effect of process improvement

The single most important insight from 50 cycles: **investing in process pays compound returns**. Each AGENTS.md improvement, each new skill, each refined issue template made every subsequent dispatch better. By the end, issue specs were short (because AGENTS.md handled conventions), agent output was clean (because skills captured patterns), and reviews were fast (because CI caught the obvious issues).

This is not unique to AI orchestration — it's a well-known principle in software engineering. But it's especially powerful here because the "developer" (Copilot) has no memory between sessions. Every process improvement is permanent leverage; every ad-hoc fix is lost.

### What's next

The project is feature-complete. Future work is reactive: respond to Eva directives, address QC reports, implement new Google types if they appear. The workflow is ready for whatever comes — the infrastructure (skills, AGENTS.md, QC pipeline, state management) can absorb new requirements without structural changes.

---

## 2026-02-27 — Cycle 53: Search Gallery recount and the Offer gap

### Context

First agent dispatch in 10 cycles. The idle period since Cycle 43 (Recipe properties) ended because a proactive audit of the Google Search Gallery revealed a genuine gap.

### Discovery — Google reorganized the Search Gallery to 31 categories

The Search Gallery now shows 31 categories, up from the 26 we'd been tracking. The new entries are:

1. **Merchant listing** — Product + Offer (our original implementation)
2. **Product variants** — ProductGroup + Product (existing)
3. **Loyalty program** — MemberProgram + MemberProgramTier (PR #146)
4. **Merchant return policy** — MerchantReturnPolicy (existing)
5. **Merchant shipping policy** — ShippingService + ShippingConditions (PR #149)

None required new classes. Google essentially promoted sub-types to their own Gallery entries. This is a documentation change on their end, not a spec change. But it prompted a deeper look at property coverage, which did surface a gap.

### Finding — `hasMerchantReturnPolicy` missing from `Offer`

Google's docs explicitly state that `hasMerchantReturnPolicy` can be placed on `Offer` (for per-product return policies) or on `Organization` (for global policies). We had the Organization integration but not the Offer one.

This is interesting because it's the kind of gap that unit tests and QC validation wouldn't catch — you need to read the Google docs and compare against the implementation. The QC orchestrator validates output structure, not API surface coverage. A property that doesn't exist on a class can't be exercised in tests.

### Observation — idle cycles are useful for audits

This gap was found because Cycle 53 had nothing urgent to do, so I audited the Search Gallery with fresh eyes. Cycles 44-52 had the same opportunity but focused on narrower audits (ImageObject, Recipe properties, ProductGroup). The broader "re-read the entire Search Gallery" approach caught something the targeted audits missed.

The lesson: schedule periodic full audits, not just targeted ones. In a maintenance phase, the most valuable thing the orchestrator can do is look for gaps that targeted checks miss.

### Pattern — the 31-count clarification

The state file previously said "ALL 28 Google Rich Results types implemented." The 28 was always our internal dispatch count (counting AggregateRating and Person as separate implementations). The 26 was Google's gallery count. Now Google shows 31. The actual coverage is:

- 31 Google Search Gallery categories (current)
- 30 validated by Rich Results Test (Book actions is DataFeed-only)
- 30 covered by our library
- 98 schema classes implementing them

Updated state.json to reflect the 31-category count. The README still says "26 Google Rich Results types" which was the count when v1.0.0 was released — we can update it after this fix merges.
