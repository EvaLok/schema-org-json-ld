...
## 2026-02-26 — Cycle 39: PHPStan level 6 and incremental quality improvement

### Context

Eva merged [PR #203](https://github.com/EvaLok/schema-org-json-ld/issues/203) (PHPStan level 5) between cycles. This cycle explored level 6 and dispatched the upgrade.

### Decision — proactive level upgrade

With PHPStan now in CI, the natural next step was assessing the gap to level 6. Running it locally showed only 14 errors — all `missingType.iterableValue` or `missingType.parameter`. This is a small, well-scoped task: add PHPDoc `@param` annotations with array value types, plus one missing `bool` type hint. Low risk, high value for code quality.

### Pattern — inline @var vs method-level @param

The codebase had two PHPDoc patterns for array types on constructor parameters:
1. Inline `/** @var Type[] $param */` directly above the parameter (old pattern)
2. Method-level `@param Type[] $param` in a PHPDoc block above the constructor (correct pattern)

PHPStan level 6 only recognizes the method-level `@param` pattern. The inline `@var` annotations were insufficient for static analysis. Updated AGENTS.md reference examples and guidance to use the correct pattern going forward.

### Observation — CI workflow approval for bot PRs

The "Test and Build" CI workflow shows `action_required` for PRs created by `copilot-swe-agent[bot]`. This appears to be GitHub's approval requirement for workflows triggered by bot accounts. The `claude-review` workflow runs fine (probably uses a different trigger path). This didn't block the merge since I verified everything locally, but it means future Copilot PRs won't have automated Test and Build CI unless someone approves the workflow run manually.

### Observation — quality toolchain complete

The project now has a comprehensive quality toolchain:
- **PHPUnit 10.x**: 313 tests, 1642 assertions
- **PHP-CS-Fixer**: Enforced code style (PER-CS2.0)
- **PHPStan level 6**: Static type analysis with no errors

This is a mature setup for a PHP library. Level 7+ would add more checks but would likely require significant refactoring of the generic array types in JsonLdGenerator.
