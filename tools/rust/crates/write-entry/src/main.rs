...
fn execute_patch_pipeline(args: &PatchPipelineArgs, repo_root: &Path) -> Result<PathBuf, String> {
    ...
    let in_flight_value = match args.in_flight {
        ...
        None => {
            let state = load_worklog_state(repo_root, false).ok().flatten();
            state_extra_in_flight_sessions(state.as_ref()).ok()
        }
    };
...
fn apply_worklog_auto_derivations(
    ...
    let cycle_receipt_before = if args.auto_receipts || args.auto_self_modifications {
    ...
    let cycle_receipt_entries = if args.auto_receipts || args.auto_self_modifications {
        ...
        Some(
            derive_cycle_receipt_entries(repo_root, cycle, cycle_receipt_before.as_deref())
    ...
    if args.auto_receipts {
        ...
        input.receipt_note = Some(match derive_receipt_scope_note(
            cycle,
            state.as_ref(),
            entries,
            cycle_receipt_before.as_deref(),
        ) {
            Ok(note) => note,
            Err(error) => {
                warnings.push(format!(
                ...
                fallback_receipt_scope_note(cycle, entries, cycle_receipt_before.as_deref())
            }
        });
    }
    Ok(warnings)
...
fn derive_receipt_scope_note(
    ...
    cycle_receipt_before: Option<&str>,
) -> Result<String, String> {
    ...
    let mut scope = match cycle_receipt_before {
        Some(timestamp) => format!("cycle {cycle} commits before {timestamp} (cycle-complete)"),
    ...
    Ok(format_receipt_scope_note(cycle, &scope, cycle_receipt_before))
}
...
fn fallback_receipt_scope_note(
    ...
    format_receipt_scope_note(cycle, &scope, cycle_receipt_before)
}

fn cycle_receipts_scope_command(cycle: u64, cycle_receipt_before: Option<&str>) -> String {
    match cycle_receipt_before {
        Some(timestamp) => format!("cycle-receipts --cycle {cycle} --before {timestamp}"),
        None => format!("cycle-receipts --cycle {cycle}"),
    }
}

fn format_receipt_scope_note(cycle: u64, scope: &str, cycle_receipt_before: Option<&str>) -> String {
    let command = cycle_receipts_scope_command(cycle, cycle_receipt_before);
...
fn derive_cycle_receipt_entries(
    ...
    let output = command
        ...
        .output()
        .map_err(|error| {
            let before_fragment = before
                .map(|timestamp| format!(" --before {}", timestamp))
                .unwrap_or_default();
            format!(
...
mod tests {
    ...
    fn fallback_receipt_scope_note_uses_factual_auto_generated_message() {
        let entries = vec![CycleReceiptJsonEntry {
            ...
            url: Some("https://github.com/EvaLok/schema-org-json-ld/commit/abcdef1234567890".to_string()),
            _aliases: Vec::new(),
        }];
    ...
    #[test]
    fn worklog_auto_receipts_pass_before_timestamp_when_cycle_complete_exists() {
        ...
        assert!(argv.contains("--before"));
        assert!(argv.contains("2026-03-06T04:00:00Z"));
        assert!(content.contains(
            "Scope: cycle 154 commits before 2026-03-06T04:00:00Z (cycle-complete)"
        ));
        assert!(content.contains(
            "Receipt table auto-generated by `cycle-receipts --cycle 154 --before 2026-03-06T04:00:00Z`."
        ));
    }
