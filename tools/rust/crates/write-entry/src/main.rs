...
fn resolve_worklog_input(args: &WorklogArgs, repo_root: &Path) -> Result<WorklogInput, String> {
        ...
        let input = WorklogInput {
            ...
            current_state: CurrentState {
                in_flight_sessions: match args.in_flight {
                    Some(value) => value,
                    None => state_in_flight(state.as_ref())?,
                },
    ...
    let input = WorklogInput {
        ...
        current_state: CurrentState {
            in_flight_sessions: state_in_flight(state.as_ref())?,
            pipeline_status: resolve_pipeline_status(args, repo_root, state.as_ref())?,
...
fn state_in_flight(state: Option<&StateJson>) -> Result<u64, String> {
    let state = state.ok_or_else(|| {
        "docs/state.json is required to populate in-flight agent sessions".to_string()
    })?;
    let in_flight = match state.in_flight_sessions {
        Some(value) => value,
        None => state
            .agent_sessions
            .iter()
...
fn derive_receipt_scope_note(
    ...
    let mut scope = format!("cycle {cycle} commits through cycle-complete");
    if !scope_bits.is_empty() {
        scope.push_str(" — ");
        scope.push_str(&scope_bits.join("; "));
    }

    Ok(format_receipt_scope_note(cycle, &scope))
}

fn fallback_receipt_scope_note(cycle: u64, entries: &[CycleReceiptJsonEntry]) -> String {
    let mut scope = format!("cycle {cycle} commits through cycle-complete");
    if let Some(event_summary) = summarize_receipt_events(entries) {
        scope.push_str(" — ");
        scope.push_str(&event_summary);
    }
    format_receipt_scope_note(cycle, &scope)
}

fn format_receipt_scope_note(cycle: u64, scope: &str) -> String {
    format!(
        "Scope: {scope}. Receipt table auto-generated by `cycle-receipts --cycle {cycle}`."
    )
}
...
mod tests {
    ...
    fn fallback_receipt_scope_note_uses_factual_auto_generated_message() {
        ...
        let note = fallback_receipt_scope_note(154, &entries);

        assert!(note.contains("Scope: cycle 154 commits through cycle-complete"));
