...
fn get_extra_i64(state: &StateJson, key: &str) -> Option<i64> {
...
fn check_chronic_category_responses_freshness(state: &StateJson) -> CheckResult {
    ...
    {
        Some(entries) => entries,
        None => return pass("chronic_category_responses freshness"),
    };

    if entries.is_empty() {
        return pass("chronic_category_responses freshness");
    }
    ...
    let refreshed_cycle = match parse_cycle_marker(last_refreshed) {
        Some(value) => value,
        None => {
            return warn(
                "chronic_category_responses freshness",
    ...
    for (entry_index, entry) in entries.iter().enumerate() {
        ...
        let Some(updated_cycle_value) = entry.get("updated_cycle") else {
            continue;
        };
        let updated_cycle = match updated_cycle_value.as_i64() {
            Some(value) => value,
            None => {
                return warn(
                    "chronic_category_responses freshness",
                    format!(
                        "invalid field: review_agent.chronic_category_responses.entries[{}].updated_cycle must be a number",
                        entry_index
                    ),
                )
            }
        };
        if updated_cycle > refreshed_cycle {
            failures.push(format!(
                "category '{}' updated_cycle({}) exceeds field_inventory.fields.review_agent.chronic_category_responses.last_refreshed({})",
                category, updated_cycle, refreshed_cycle
            ));
        }
    }

    if failures.is_empty() {
        pass("chronic_category_responses freshness")
    } else {
        fail("chronic_category_responses freshness", failures.join("; "))
    }
}

fn check_chronic_verification_deadline(state: &StateJson) -> CheckResult {
    let review_agent = match state.extra.get("review_agent") {
        Some(value) => value,
        None => {
            return warn(
    ...
    {
        Some(entries) => entries,
        None => return pass("chronic_verification_deadline"),
    };
    ...
    for (entry_index, entry) in entries.iter().enumerate() {
        ...
        let added_cycle = match entry.get("added_cycle").and_then(Value::as_i64) {
            ...
            None => {
                return warn(
                    "chronic_verification_deadline",
                    format!(
                        "missing field: review_agent.chronic_category_responses.entries[{}].added_cycle",
                        entry_index
                    ),
                )
            }
        ...
        let cycles_since_added = current_cycle - added_cycle;
        if cycles_since_added < 5 {
            continue;
        }

        let review_count = match count_category_reviews_since(history, category, added_cycle) {
            Ok(value) => value,
            Err(details) => return warn("chronic_verification_deadline", details),
        };

        if review_count >= 3 {
            failures.push(format!(
                "Chronic category '{}' has verification_cycle: null, was added {} cycles ago, and has appeared in {} reviews since. Either verify the fix works and set verification_cycle, or re-file a stronger structural fix.",
                category, cycles_since_added, review_count
            ));
        }
    }
...
fn count_category_reviews_since(
    history: &[Value],
    category: &str,
    added_cycle: i64,
) -> Result<usize, String> {
    ...
    for (history_index, entry) in history.iter().enumerate() {
        let cycle = entry.get("cycle").and_then(Value::as_i64).ok_or_else(|| {
            format!(
                "missing field: review_agent.history[{}].cycle",
                history_index
            )
        ...
        if cycle >= added_cycle && review_history_entry_mentions_category(entry, category)? {
            review_count += 1;
        }
    }
...
fn review_history_entry_mentions_category(entry: &Value, category: &str) -> Result<bool, String> {
    let categories_match = entry
        .get("categories")
        .and_then(Value::as_array)
        .map(|categories| {
            categories
    ...
    let findings = match entry.get("findings") {
...
mod tests {
    ...
    #[test]
    fn last_cycle_summary_receipts_skips_validation_for_open_current_cycle() {
        let repo_root = temp_repo_root("summary-receipts-open-cycle");
        init_git_repo(&repo_root);
        commit_receipt(
            &repo_root,
            "notes.txt",
    ...
    fn get_metric_i64_reads_top_level_extra_fields() {
        ...
        assert_eq!(get_extra_i64(&state, "total_dispatches"), Some(3));
        assert_eq!(get_extra_i64(&state, "merged"), Some(1));
    }
    ...
    fn chronic_category_responses_freshness_passes_when_entries_are_not_newer_than_marker() {
        let mut value = minimal_valid_state();
        value["field_inventory"]["fields"]["review_agent.chronic_category_responses"] = json!({
            "last_refreshed": "cycle 10"
        });
        value["review_agent"]["chronic_category_responses"] = json!({
            "entries": [
    ...
    #[test]
    fn chronic_category_responses_freshness_fails_when_entry_is_newer_than_marker() {
        let mut value = minimal_valid_state();
        value["field_inventory"]["fields"]["review_agent.chronic_category_responses"] = json!({
            "last_refreshed": "cycle 448"
        });
        value["review_agent"]["chronic_category_responses"] = json!({
            "entries": [
                {
        ...
        assert_eq!(check.status, CheckStatus::Fail);
        let details = check.details.as_deref().unwrap_or_default();
        assert!(details.contains("process-adherence"));
        assert!(details.contains("450"));
        assert!(details.contains("448"));
    }

    #[test]
    fn chronic_category_responses_freshness_ignores_legacy_entries_without_updated_cycle() {
        let mut value = minimal_valid_state();
        value["field_inventory"]["fields"]["review_agent.chronic_category_responses"] = json!({
    ...
    #[test]
    fn chronic_verification_deadline_fails_when_null_verification_is_old_and_recurring() {
        let mut value = minimal_valid_state();
        ...
        assert!(details.contains("5 cycles ago"));
        assert!(details.contains("3 reviews"));
    }

    #[test]
    fn chronic_verification_deadline_passes_when_null_verification_is_recent() {
        let mut value = minimal_valid_state();
        value["last_cycle"]["number"] = json!(20);
        value["review_agent"]["history"] = json!([
            {
    ...
    #[test]
    fn chronic_verification_deadline_passes_when_verification_cycle_is_set() {
        let mut value = minimal_valid_state();
        value["last_cycle"]["number"] = json!(20);
        value["review_agent"]["history"] = json!([
            {
    ...
    fn run_checks_includes_forward_work_and_backfill_checks() {
        ...
        assert_eq!(
            ...
            Some("chronic_category_responses freshness")
        );
        assert_eq!(
        ...
        assert_eq!(
            report.checks.get(13).map(|check| check.status),
