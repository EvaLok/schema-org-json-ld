...
fn apply_eva_processing(
    ...
    let cycle_marker = format!("cycle {}", current_cycle);
    ...
    if no_changes {
        ...
        let provided_remaining = remaining_open.ok_or_else(|| {
            "--remaining-open is required unless --no-changes is used".to_string()
        })?;
        ...
        let mut next_closed_prior =
            read_issue_array(state, "/eva_input_issues/closed_prior_cycles")?;
...
fn build_commit_message(
    ...
    if no_changes {
        format!("state(process-eva): no changes [cycle {}]", current_cycle)
    } else {
        format!(
...
mod tests {
    ...
    fn write_temp_state(state: &Value) -> PathBuf {
        ...
        let repo_root = std::env::temp_dir().join(format!("process-eva-test-{}", run_id));
    ...
    fn current_cycle_matches_last_cycle_number_from_state_file() {
        ...
        assert_eq!(current_cycle(&repo_root).unwrap(), 167);
