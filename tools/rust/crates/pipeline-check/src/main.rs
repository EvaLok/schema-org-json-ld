...
const POST_DISPATCH_DELTA_FIRST_APPLICABLE_PREVIOUS_CYCLE: u64 = 515;
...
mod tests {
    ...
    fn post_dispatch_delta_present_passes_when_previous_cycle_worklog_contains_section() {
        ...
        fs::write(
            root.join("docs/state.json"),
            json!({
                "last_cycle": {"number": 515},
                "cycle_phase": {"cycle": 516}
            })
            .to_string(),
        )
        .unwrap();
        ...
        fs::write(
            root.join("docs/worklog/2026-04-19/094529-cycle-515-summary.md"),
            "# Cycle 515 — 2026-04-19 09:45 UTC\n\n## Post-dispatch delta\n\n- **In-flight agent sessions**: 1\n- **Dispatch count**: 1 dispatch\n- **Last-cycle summary**: 1 dispatch, 0 merges\n",
        )
        .unwrap();
        commit_all(&root, "state(record-dispatch): #2594 dispatched [cycle 515]");
        ...
        assert!(step
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("cycle 515 worklog includes post-dispatch delta"));
    }
    ...
    fn post_dispatch_delta_present_skips_when_previous_cycle_has_no_dispatch_receipt() {
        ...
        .unwrap();
        commit_all(&root, "state(cycle-start): begin cycle 516 [cycle 516]");
        ...
        assert!(step
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("no record-dispatch receipts found for cycle 515"));
    }
