...
impl CommandRunner for ProcessRunner {
    ...
    fn fetch_audit_inbound_issues(&self) -> Result<Vec<AuditInboundIssue>, String> {
        let output = Command::new("gh")
            ...
            .arg("-f")
            .arg(format!(
                "q=repo:{MAIN_REPO} is:issue in:title \"[audit-inbound]\""
            ))
            .arg("-f")
            .arg("per_page=100")
            .output()
        ...
        for page in pages {
            let items = page
                ...
                .ok_or_else(|| "gh api search payload missing items array".to_string())?;
            for item in items {
                let issue: AuditInboundIssue =
                    serde_json::from_value(item.clone()).map_err(|error| {
                        format!("failed to parse audit-inbound issue payload: {}", error)
                    })?;
                issues.push(issue);
            }
        }
...
fn verify_post_dispatch_delta_heading_required(repo_root: &Path) -> StepReport {
    match post_dispatch_delta_heading_required_assessment(repo_root) {
        Ok(assessment) => StepReport {
            name: POST_DISPATCH_DELTA_HEADING_REQUIRED_STEP_NAME,
            status: assessment.status,
            severity: assessment.severity,
            exit_code: None,
            detail: Some(assessment.detail),
...
fn post_dispatch_delta_heading_required_assessment(
    ...
    let Some(worklog_path) = latest_worklog_entry_for_cycle(repo_root, cycle)? else {
        return Ok(StepAssessment {
            ...
            severity: Severity::Warning,
            detail: format!("cycle {} has no worklog entry to inspect", cycle),
        });
    };
    let content = fs::read_to_string(&worklog_path)
        .map_err(|error| format!("failed to read {}: {}", worklog_path.display(), error))?;
    let has_forward_reference = content
        .lines()
        .any(|line| line.contains("`## Post-dispatch delta`"));
    let has_heading = content
        .lines()
        .any(|line| line.trim() == "## Post-dispatch delta");
    if has_forward_reference && !has_heading {
        return Ok(StepAssessment {
            status: StepStatus::Fail,
            severity: Severity::Blocking,
            detail: format!(
                "cycle {} worklog references `## Post-dispatch delta` but is missing the heading line ({})",
                cycle,
                worklog_path.display()
            ),
        });
    ...
    let detail = if has_forward_reference {
        format!(
            "cycle {} worklog includes the required Post-dispatch delta heading ({})",
            cycle,
            worklog_path.display()
        )
    } else {
        format!(
            "cycle {} worklog has no Post-dispatch delta forward reference ({})",
            cycle,
            worklog_path.display()
        )
    };
...
// First cycle whose worklog was generated by the post-dispatch-delta-aware
// record-dispatch path (PR #2590, merged cycle 515). Earlier cycles' worklogs
...
fn audit_inbound_lifecycle_assessment(
    ...
    if audit_processed.is_empty() {
        return Ok(StepAssessment {
            status: StepStatus::Pass,
            severity: Severity::Blocking,
            detail: "no audit dispositions to verify".to_string(),
        });
    }

    let issues = runner.fetch_audit_inbound_issues()?;
    let missing = audit_processed
        .iter()
        .filter(|audit_issue| {
            !issues
                .iter()
                .any(|issue| audit_inbound_issue_matches(issue, **audit_issue))
        })
        .copied()
        .collect::<Vec<_>>();
    ...
    if missing.is_empty() {
        return Ok(StepAssessment {
            ...
            severity: Severity::Blocking,
            detail: format!(
                "verified {} audit disposition acknowledgments",
                audit_processed.len()
            ),
        });
    }
    ...
    let mut detail = vec![format!(
        "missing {} audit-inbound acknowledgment(s):",
        missing.len()
    )];
    detail.extend(missing.iter().map(|audit_issue| {
        format!(
            "- audit #{audit_issue} missing; suggested title: {}",
            audit_inbound_suggested_title(*audit_issue)
        )
    }));
...
mod tests {
    ...
    #[test]
    fn post_dispatch_delta_present_skips_cycles_predating_structural_fix() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
            "pipeline-check-post-dispatch-delta-pre-fix-{}",
            run_id
        ));
        init_git_repo(&root);
        fs::create_dir_all(root.join("docs")).unwrap();
        fs::write(
            root.join("docs/state.json"),
            json!({
                "last_cycle": {"number": 514},
                "cycle_phase": {"cycle": 515}
            })
            .to_string(),
        )
        .unwrap();
        ...
        fs::write(
            root.join("docs/worklog/2026-04-18/214202-cycle-514-summary.md"),
    ...
    #[test]
    fn post_dispatch_delta_heading_required_fails_when_forward_reference_lacks_heading() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
            "pipeline-check-post-dispatch-heading-fail-{}",
            run_id
        ));
        init_git_repo(&root);
        fs::create_dir_all(root.join("docs")).unwrap();
        fs::write(
            root.join("docs/state.json"),
            json!({
                "last_cycle": {"number": 525},
                "cycle_phase": {"cycle": 525}
            })
            .to_string(),
        )
        .unwrap();
        fs::create_dir_all(root.join("docs/worklog/2026-04-21")).unwrap();
        fs::write(
            root.join("docs/worklog/2026-04-21/214438-cycle-525-summary.md"),
        ...
        .unwrap();
        commit_all(&root, "docs(cycle-525): freeze worklog [cycle 525]");

        let step = verify_post_dispatch_delta_heading_required(&root);
        ...
        assert!(step
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("references `## Post-dispatch delta` but is missing the heading"));
    }

    #[test]
    fn post_dispatch_delta_heading_required_passes_when_heading_is_present() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
            "pipeline-check-post-dispatch-heading-pass-{}",
            run_id
        ));
        init_git_repo(&root);
        fs::create_dir_all(root.join("docs")).unwrap();
        ...
        assert!(step
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("includes the required Post-dispatch delta heading"));
    }
    ...
    fn worklog_immutability_passes_when_gate_failure_honesty_documents_fail_to_fail_pass() {
        ...
        fs::write(
            &worklog,
            format!(
                "# Cycle 410\n\n## Cycle state\n\n- **Pipeline status**: FAIL→PASS (C5.5 initially failed: {initial_status}; resolved by re-run)\n"
            ),
        )
        .unwrap();
    ...
    #[test]
    fn audit_inbound_lifecycle_passes_when_all_dispositions_are_acknowledged() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
        ...
        impl CommandRunner for Runner {
            ...
            fn fetch_audit_inbound_issues(&self) -> Result<Vec<AuditInboundIssue>, String> {
                Ok(vec![
                    AuditInboundIssue {
                        title: "[audit-inbound] Acknowledge audit #382".to_string(),
                        body: String::new(),
                        state: "open".to_string(),
                    },
                    AuditInboundIssue {
                        title: "[audit-inbound] Acknowledge audit follow-up".to_string(),
                        body: "Formal acknowledgment for audit #383".to_string(),
        ...
        assert_eq!(
            step.detail.as_deref(),
            Some("verified 3 audit disposition acknowledgments")
        );
    }

    #[test]
    fn audit_inbound_lifecycle_fails_when_disposition_acknowledgment_is_missing() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
        ...
        assert!(step
            ...
            .as_deref()
            .unwrap_or_default()
            .contains(&audit_inbound_suggested_title(383)));
    }

    #[test]
    fn audit_inbound_lifecycle_passes_when_no_audit_dispositions_exist() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
        ...
        impl CommandRunner for Runner {
            ...
            fn fetch_audit_inbound_issues(&self) -> Result<Vec<AuditInboundIssue>, String> {
                Err("audit-inbound issue fetch should be skipped".to_string())
            }
        }
    ...
    #[test]
    fn audit_inbound_lifecycle_accepts_open_and_closed_issue_matches() {
        static COUNTER: AtomicU64 = AtomicU64::new(0);
        let run_id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let root = std::env::temp_dir().join(format!(
        ...
        impl CommandRunner for Runner {
            ...
            fn fetch_audit_inbound_issues(&self) -> Result<Vec<AuditInboundIssue>, String> {
                Ok(vec![
                    ...
                    AuditInboundIssue {
                        title: "[audit-inbound] Acknowledge closed audit".to_string(),
                        body: "Resolved via audit #383".to_string(),
                        state: "closed".to_string(),
                    },
                ])
            }
