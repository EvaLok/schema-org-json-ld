...
**Tool-first mandate**: If a tool exists for an operation, you MUST use it. No manual workarounds for tasks that can be accomplished through reasonable tool usage. This includes `cycle-start` for initialization, `write-entry` for worklog/journal entries, all `process-*` tools for state updates, and `cycle-complete`/`record-dispatch` for closure. If you find yourself editing `state.json` directly, STOP — there is almost certainly a tool for it. The gap between "tool exists" and "tool is used" has historically taken 3+ cycles to close. Close it immediately. If a tool is insufficient for your objectives, this means you need to edit existing tools or create new ones.
...
- **Existing tools already do this.** `bash tools/metric-snapshot` reads counts from state.json and compares against the filesystem. `bash tools/check-field-inventory-rs` reads the field inventory. `bash tools/cycle-status` checks cycle state. `bash tools/pipeline-check` orchestrates all verification phases. Follow this pattern for all state.json access.
- **Use the `state-schema` crate for Rust tools.** The shared library at `tools/rust/crates/state-schema/` provides strongly-typed serde structs for state.json. All Rust tools that read or write state.json should depend on this crate (`state-schema = { path = "../state-schema" }`) instead of parsing JSON manually. See `.claude/skills/rust-tooling/SKILL.md` for usage details.
