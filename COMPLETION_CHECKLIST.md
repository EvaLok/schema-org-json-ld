...
| Copilot task dispatched | `record-dispatch` | `bash tools/record-dispatch --issue N --title "..." --model gpt-5.4` (add `--addresses-finding <finding-id>` when the dispatch addresses a specific review finding) |
...
2. **Provide specific review targets.** List each area to examine with explicit instructions on what to look for:
   ...
   - **Process adherence**: did the orchestrator follow its own checklist? Did it use tools when tools exist? Did it skip steps? **Temporal ordering**: verify that all startup step comments (Steps 0, 0.5, 0.6, 1, 1.1, 2, 3) were posted before any close-out step comments (Steps C1+). Check the `created_at` timestamps — startup steps appearing after close-out steps is a retroactive posting violation (per [audit #273](https://github.com/EvaLok/schema-org-json-ld-audit/issues/273)).
